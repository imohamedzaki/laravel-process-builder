...
import { LaneBandNode, LANE_BAND_HEIGHT, LANE_BAND_WIDTH, type LaneBandNodeData } from '@/nodes/LaneBandNode';
...
function CanvasInner(): JSX.Element {
    ...
    const { nodes: processNodes, edges: processEdges, lanes: processLanes, selectedNodeId, addNode, moveNode, selectNode, removeNode, addEdge, removeEdge, assignNodeToLane } = useProcessEditorStore();
    const [quickAdd, setQuickAdd] = useState<QuickAddState | null>(null);
    const sortedLanes = useMemo(() => [...processLanes].sort((a, b) => a.order - b.order), [processLanes]);
    const laneBandNodes: Node[] = useMemo(() => sortedLanes.map((lane, index) => ({ id: `lane-band_${lane.id}`, type: 'laneBandNode', position: { x: -LANE_BAND_WIDTH / 2, y: index * LANE_BAND_HEIGHT }, draggable: false, selectable: false, deletable: false, zIndex: -1, data: { name: lane.name, color: lane.color } satisfies LaneBandNodeData })), [sortedLanes]);
    ...
    const handleNodeDragStop = useCallback((_event: unknown, node: Node) => { moveNode(node.id, node.position); if (sortedLanes.length) { const lane = sortedLanes[Math.max(0, Math.floor(node.position.y / LANE_BAND_HEIGHT))] ?? null; assignNodeToLane(node.id, lane?.id ?? null); } }, [moveNode, assignNodeToLane, sortedLanes]);
    ...
    function addConnectedNode(type: NodeType): void {
        ...
        addNode({ id, type, position: quickAdd.flow, data: defaultData(type) });
    ...
    const handleDrop = useCallback((event: React.DragEvent) => { event.preventDefault(); const type = event.dataTransfer.getData(PALETTE_DRAG_MIME) as NodeType; if (!type) return; const id = nextNodeId(type); addNode({ id, type, position: screenToFlowPosition({ x: event.clientX, y: event.clientY }), data: defaultData(type) }); selectNode(id); }, [addNode, screenToFlowPosition, selectNode]);
